...
sub scandir ()
{
  ...
  if (defined ($package->{'version'})) {
	  ...
	  warn ("unable to find either version or revision for package; using 0");
	  $package->{'version'} = 0;
      }
  }      
...
sub makeroot ()
{
    ...
    my %curdeps = ();
    my $depfile = "$buildroot/var/db/package-list";
    ...
    system ('mkdir', '-p', "$buildroot/var/db");
    if ($? != 0) { die (&checkret ($?)); }

    my $pfilename = "$buildroot/var/db/package-list";
...
sub build () 
{
  ...
  if (($target eq 'all') || ($target eq 'headers')) {
    ...
    $ENV{'UNAME_RELEASE'} = '5.3';
    $ENV{'PATH'} = '/sbin:/usr/sbin:/bin:/usr/bin:/usr/local/bin';
    print 'UNAME_SYSNAME=Rhapsody UNAME_RELEASE=5.3 PATH=/sbin:/usr/sbin:/bin:/usr/bin:/usr/local/bin ';
